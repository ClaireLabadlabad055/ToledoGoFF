...
export default function VendorMessageThreadScreen() {
  ...
  const loadMessages = useCallback(async () => {
    ...
    } catch (loadError: any) { setError(loadError.message ?? 'Unable to load this conversation.'); }
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`vendor-conversation-${conversationId}`).on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `conversation_id=eq.${conversationId}` }, loadMessages).subscribe();
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setDraft('');
    } catch (sendError: any) { setError(sendError.message ?? 'Unable to send your message.'); }
    finally { setSending(false); }
  };

  return <View style={styles.container}><StatusBar barStyle="light-content" backgroundColor="#7C2D12" /><SafeAreaView style={styles.safeArea}><View style={styles.header}><TouchableOpacity style={styles.backButton} onPress={() => router.back()} accessibilityLabel="Back"><Feather name="arrow-left" size={19} color="#292524" /></TouchableOpacity><View style={styles.headerCopy}><Text style={styles.eyebrow}>VENDOR MESSAGES</Text><Text style={styles.title}>{customerName || 'Customer'}</Text><Text style={styles.subtitle}>Coordinate custom requests and order details.</Text></View><Feather name="message-circle" size={25} color="#FED7AA" /></View>{error ? <Text style={styles.error}>{error}</Text> : null}{loading ? <ActivityIndicator color="#C2410C" style={styles.loader} /> : <ScrollView contentContainerStyle={styles.messages} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); loadMessages(); }} tintColor="#C2410C" />}>{messages.length === 0 ? <View style={styles.empty}><Feather name="send" size={28} color="#C2410C" /><Text style={styles.emptyTitle}>Start the conversation</Text><Text style={styles.emptyText}>Clarify quantities, flavors, timing, payment, and pickup details.</Text></View> : messages.map((message) => <View key={message.id} style={[styles.messageBubble, message.sender_id === userId ? styles.myMessage : styles.theirMessage]}><Text style={[styles.messageText, message.sender_id === userId && styles.myMessageText]}>{message.body}</Text><Text style={[styles.messageTime, message.sender_id === userId && styles.myMessageTime]}>{new Date(message.created_at).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}</Text></View>)}</ScrollView>}<KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={8} style={styles.composer}><TextInput style={styles.input} value={draft} onChangeText={setDraft} placeholder="Reply to customer..." placeholderTextColor="#A8A29E" multiline maxLength={2000} /><TouchableOpacity style={[styles.sendButton, !draft.trim() && styles.sendDisabled]} onPress={sendMessage} disabled={!draft.trim() || sending} accessibilityLabel="Send message"><Feather name="send" size={17} color="#FFFFFF" /></TouchableOpacity></KeyboardAvoidingView></SafeAreaView></View>;
}
